...
class HebbianMap2d(nn.Module):
	...
	def __init__(self,
		...
		self.N_hid = N_hid
		self.krotov = True
		if self.N_hid == 0: self.krotov = False
	...
	def forward(self, x):
		if self.krotov:
			...
			self.tot_input = torch.mm(sig * torch.abs(self.weight).pow(self.p - 1), inputs)
			y = torch.transpose(self.tot_input, 0, 1)
		else:
	...
	def update(self, x):
		...
		if self.krotov:
			...
			y1 = torch.zeros((self.N_hid, x_unf.shape[0]))  # g(Q)
			y1[y[self.N_hid - 1, :], np.arange(x_unf.shape[0])] = 1.0  # g(max_activation in I) = 1
			y1[y[self.N_hid - k], np.arange(x_unf.shape[0])] = -delta  # g(second max activation) = -delta

			xx = torch.sum(torch.mul(y1, self.tot_input), 1)  # g(Q) * <W, v>
			ds = torch.matmul(y1, torch.transpose(x, 0, 1)) - \
				 torch.mul(xx.reshape(xx.shape[0], 1).repeat(1, self.in_channels),
																			 self.weight)
	...
	# def forward_krotov(self, inputs, synapses, p, N_batch):
	# 	N_in = inputs.shape[1]
	...
	# 	y1 = torch.zeros((self.N_hid, N_batch)).to(device)  # g(Q)
	...
	# 	ds = torch.matmul(y1, torch.transpose(inputs, 0, 1)) - torch.mul(xx.reshape(xx.shape[0], 1).repeat(1, N_in),
	# 																	 synapses)
	# 	nc = torch.max(torch.abs(ds))
	#
	# 	return ds, nc
	#
	#
	# def train_weights(self, x_train, N_hid, N_batch, Nep=Nep, mu=0, sigma=1, device="cpu"):
	...
	# 			ds, nc = self.forward(inputs, synapses, p, N_hid, N_batch)
	#
	# 			if nc < prec:
	# 				nc = prec
